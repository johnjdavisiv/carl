function [vm_final_logical, vm_EC_raw, vm_EC_LR_raw, vm_final] = CARLclassify_interrogate(vm,device_location, continuity, fs)

%{
Run CARLclassifier and return intermediate results. Useful for plotting and diagnostics
...
continuity: int
        Minimum of seconds of continuous running. CARL will ignore any bouts of running that are
        ...
        the loss of precision in estimating the dominant frequency.

fs: int or float
        Sample frequency of original signal, in Hz. CARL has been validated on data ranging from 20
        to 256 Hz and should work for arbitrarily high sample frequencies. CARL will raise a warning
...
This is NOT the "official" CARL classifier. This is a helper function for debugging and making plots
to understand which parts of a signal pass the amplitude, continuity, and logistic parts of the
...
%expand to integer number of second by reflecting data (will truncate again at end)
...
vm_EC_LR_raw = vm_EC_raw;
vm_EC_LR(~logical(vm_LR_logical)) = NaN;
vm_EC_LR_raw(~logical(vm_LR_logical)) = NaN;
...
%Return final
vm_final = vm_EC_LR_raw;
vm_final(~vm_final_logical) = NaN;
